	...
	*/
	namespace DecompressUtility {
		...
		void Decompress(const char* filename, const ll Filesize) {
			...
			fl.insert(fl.find_last_of("\\") + 1, "Huff_decode");
			cout << string(fl.substr(0, fl.length() - 6)) << "\n";
			FILE* optr = fopen(string(fl.substr(0, fl.length() - 6)).c_str(), "wb");
			...
			char ch, counter = 7;
			ll size = 0;
			const ll filesize = Filesize - headersize - 34;
			Node* traverse = root;
			ch = fgetc(iptr);
			while (size != filesize) {
				while (counter >= 0) {
